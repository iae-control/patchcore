#!/usr/bin/env python3
"""PatchCore v5b-FULL — Train ALL 5 groups sequentially.
...
Each group uses the same date-balanced strategy:
...
# Which groups to train (skip group 1 since already done)
GROUPS_TO_TRAIN = [2, 3, 4, 5]
...
# ===== FEATURE EXTRACTOR (shared across groups) =====
...
def generate_heatmap_zscore(img_path, memory_bank, extractor, spatial_size, output_path,
    ...
    plt.suptitle(f"PatchCore v5b-FULL | {TARGET_SPEC}/group_{group_id} | {img_path.name}", fontsize=13)
    plt.tight_layout()
...
def train_group(group_id, date_folders, defect_folder, extractor, spatial_size):
    """Train a single group end-to-end."""
    ...
    plt.title(f"v5b-FULL | {TARGET_SPEC}/group_{group_id} | {n_images} imgs | bank={memory_bank.shape[0]}")
...
def main():
    t_start = time.time()
    print("=" * 60)
    print("PatchCore v5b-FULL — ALL GROUPS TRAINING")
    print("=" * 60)
    ...
    # Discover folders (shared across groups)
    ...
    print(f"\n{'='*60}")
    print("ALL GROUPS TRAINING COMPLETE")
    print(f"{'='*60}")
