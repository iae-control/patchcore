#!/usr/bin/env python3
"""Experiment 3: 64x64 Tiles + 0630 Only + Z-Score.

더 작은 타일(64x64)로 결함 해상도 향상.
128x128에서 세로 균열이 global avg pool에 묻혔을 가능성.
64x64면 타일 30x18=540개/이미지, 균열이 차지하는 비율이 높아짐.
"""
import os
...
OUTPUT_DIR = Path("/home/dk-sdd/patchcore/output_exp3_tile64")
TARGET_SPEC = "596x199"
TRAIN_DATE = "20250630"
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Experiment 3: 64x64 Tiles + 0630 Only + Z-Score")
    print("=" * 60)
        ...
        for cam_id in CAM_IDS:
            ...
            for img_path in images:
                ...
                if hm_count < 20:
                    ...
                    fig.suptitle(f"Exp3 64x64 | cam{cam_id} {img_path.name}", fontsize=12)
                    plt.tight_layout()
    ...
    # Meta
    meta = {
        "experiment": "exp3_tile64_0630", "tile_size": TILE_SIZE, "tile_stride": TILE_STRIDE,
