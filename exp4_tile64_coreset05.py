#!/usr/bin/env python3
"""Experiment 4: 64x64 Tiles + 0630 + coreset ratio 0.05 + Z-Score.

exp3에서 64x64가 효과적이었음. coreset ratio를 0.01→0.05로 올려서
memory bank을 5배로 키워 정밀도 향상 시도.
"""
import os
...
OUTPUT_DIR = Path("/home/dk-sdd/patchcore/output_exp4_coreset05")
TARGET_SPEC = "596x199"
TRAIN_DATE = "20250630"
...
CAM_IDS = [1, 10]
MIRROR_CAM = 10
BATCH_SIZE = 512  # smaller tiles = more per batch
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("Experiment 4: 64x64 + Coreset 0.05 + Z-Score")
    print("=" * 60)
    ...
    # Feature extraction
    print(f"\n[2] Extracting 64x64 tile features (subsample={SUBSAMPLE_TRAIN})...")
    all_features = []
    total_images = 0
        ...
        for cam_id in CAM_IDS:
            ...
            for img_path in images:
                ...
                if hm_count < 20:
                    ...
                    fig.suptitle(f"Exp4 64x64 coreset0.05 | cam{cam_id} {img_path.name}", fontsize=12)
                    plt.tight_layout()
    ...
    # Meta
    meta = {
        "experiment": "exp4_tile64_coreset05", "tile_size": TILE_SIZE, "tile_stride": TILE_STRIDE,
